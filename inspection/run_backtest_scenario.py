...
def _build_runtime_cfg(*, config_path: str, train_size: int, calibrate_size: int, rw_floor: float, rw_power: float):
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=str,
        choices=("none", "bull_only", "bear_only", "both_sides"),
        default="none",
    )
    parser.add_argument(
        ...
        def _fixed_bet_size_bet(*, cfg, pred, bankroll_bnb: float) -> BetDecision:
            ...
            if float(fixed_bet_amount_bnb) > float(cap_bnb):
                return BetDecision(
                    ...
                    bet_cap_bnb=float(decision.bet_cap_bnb),
                    best_expected_profit_bnb=float(decision.best_expected_profit_bnb),
                    skip_reason="fixed_bet_exceeds_cap_or_bankroll",
                )
    ...
    bt_cfg = cfg.backtest
    if args.sim_size is not None:
        bt_cfg = BacktestConfig(
            simulation_size=int(args.sim_size),
            initial_bankroll_bnb=float(cfg.backtest.initial_bankroll_bnb),
        )
        bt_cfg.validate()
