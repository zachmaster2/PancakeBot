...
def _evaluate_selectors(
    *,
    rows: list[WindowRow],
    flow_profiles: list[FlowProfileSpec],
    lookbacks: list[int],
    ...
    all_profiles = ["stageb"] + [str(profile.name) for profile in flow_profiles]
...
def main() -> None:
    ...
    if not flow_profiles:
        raise InvariantError("profile_set_flow_profiles_required")
    names = [str(profile.name) for profile in flow_profiles]
    if len(names) != len(set(names)):
        raise InvariantError("profile_set_flow_profile_name_duplicate")
    ...
    for tail_offset in tail_offsets:
        ...
        metrics["stageb"] = ProfileMetric(
            per_500=float(stageb_summary["net_profit_per_500_rounds"]),
        ...
        for profile in flow_profiles:
            ...
            metrics[str(profile.name)] = ProfileMetric(
                per_500=float(flow_summary["net_profit_per_500_rounds"]),
    ...
    profile_names = ["stageb"] + [str(profile.name) for profile in flow_profiles]
