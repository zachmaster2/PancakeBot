...
def _run_subprocess(*, argv: list[str], cwd: Path) -> None:
    ...
    try:
        ...
        tail_text = ""
    raise RuntimeError(f"profile_set_penalty_shadow_refresh_subprocess_failed:\n{tail_text[-4000:]}")
...
def _shadow_argv(
    *,
    ...
    return [
        str(python_exe),
        "-m",
        "inspection.run_profile_set_penalty_shadow_recommender",
        "--compare-csv",
        str(compare_csv),
        "--name-prefix",
        str(args.name_prefix),
        ...
        "--cold-start-mode",
        str(args.cold_start_mode),
        "--cold-start-lookback",
        str(int(args.cold_start_lookback)),
        "--margin-per-500",
        ...
        "--flow-penalty-per-500",
        str(float(args.flow_penalty_per_500)),
        "--stageg2-penalty-per-500",
        str(float(args.stageg2_penalty_per_500)),
        "--output-dir",
        str(args.output_dir),
    ]
...
def main() -> None:
    ...
    if not python_exe.exists():
        raise FileNotFoundError(f"profile_set_penalty_shadow_refresh_python_missing: {python_exe}")
    ...
    summary_path = output_dir / f"{args.name_prefix}_profile_set_penalty_shadow_refresh_summary.json"
