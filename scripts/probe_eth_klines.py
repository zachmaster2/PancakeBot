"""Probe: fetch ETH/USDT 1s klines from OKX for each round.

Same timestamps as BNB data. Third cross-asset signal source.
"""
...
OUT_PATH = Path("var/eth_spot_prices.jsonl")
...
def main() -> None:
    ...
    if OUT_PATH.exists():
        ...
        print(f"Resuming: {len(done_epochs)} already fetched")
    ...
    with open(OUT_PATH, "a") as f:
        for i, rnd in enumerate(remaining):
            klines = fetch_1s_klines(rnd["lockAt"] * 1000)
            rec = {"epoch": rnd["epoch"], "lock_at": rnd["lockAt"],
                   "klines_1s": klines, "error": klines is None}
            if klines is None:
                errors += 1
