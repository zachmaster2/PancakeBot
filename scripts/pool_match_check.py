"""Compare dry mode CSV pools (RPC-observed) vs backtest pools (from stored bets).

Run this AFTER:
1. Dry mode has run for a few rounds (with the CSV pool fix)
2. Sync has been run to fetch closed round data for those epochs

It reads the dry CSV for RPC-observed pools, loads the same epochs from
closed_rounds.jsonl, computes _pools_from_bets(cutoff_ts), and shows
the difference. If the pools match, backtest is representative of live.
"""
...
def main():
    ...
    # Read dry CSV — get RPC-observed pools per epoch
    csv_pools = {}
    with open(csv_path, newline="") as f:
        ...
        for row in reader:
            ...
            # Skip rows with empty/zero pools (old format before fix)
    ...
    if not csv_pools:
        print("No non-zero pool data in CSV. Run dry mode with the CSV fix first.")
        return

    print(f"CSV epochs with pool data: {len(csv_pools)}")

    # Load closed rounds for those epochs
    ...
    for epoch in sorted(csv_pools.keys()):
        ...
        # Compute backtest pools (bets at or before cutoff_ts)
        bt_bull_wei = 0
        bt_bear_wei = 0
        for bet in rnd.bets:
