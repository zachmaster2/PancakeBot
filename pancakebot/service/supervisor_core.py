...
class SupervisorCore:
    ...
    def run(self) -> None:
        ...
        # E: this is an intentional stop (the loop exits ONLY via _stop_requested
        # -> SIGTERM/SCM stop / mode mutex / deploy). Reset the OUTER start-limit
        # counter NOW, before exit, so the next start is guaranteed a full budget
        # (deploys/admin restarts never exhaust it). No marker, no start-side
        # race. A crash never reaches here, so crash restarts still count.
        self._clear_start_limit_after_intentional_stop()
    ...
    def _clear_start_limit_after_intentional_stop(self) -> None:
        """Reset the OUTER-supervisor start-limit counter on the intentional
        stop path so the next start always has a full budget (deploys / admin
        restarts never exhaust it). Reached only when the run loop exits via
        _stop_requested; a crash never gets here, so crash restarts still count
        (crashloop guard). Best-effort — a reset failure must not block the
        clean shutdown."""
        try:
            self._platform.clear_restart_counter(self._svc_name)
            self._log("INFO", f"{self._svc_name}: intentional stop — start-limit counter cleared")
        except Exception as e:  # noqa: BLE001
