...
from pancakebot.constants import BNB_WEI, MAX_GAS_COST_CLAIM_BNB
...
def settle_from_round_data(
    *,
    ...
    if denom <= 0.0 or total_after <= 0.0:
        return SettlementResult(outcome="win", credit_bnb=-MAX_GAS_COST_CLAIM_BNB, payout_multiple_after_fee=0.0)

    mult = (total_after * (1.0 - treasury_fee_fraction)) / denom
    credit = bet_bnb * mult - MAX_GAS_COST_CLAIM_BNB
    ...
    *,
    ...
    Convention:
      ...
      - This function returns the net credit applied at claim-time:
          - WIN: bet_bnb * payout_multiple_after_fee - MAX_GAS_COST_CLAIM_BNB
          - REFUND (failed): bet_bnb - MAX_GAS_COST_CLAIM_BNB
          - LOSS: 0.0
