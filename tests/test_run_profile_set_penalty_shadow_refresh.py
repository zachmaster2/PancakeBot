...
from inspection.run_profile_set_penalty_shadow_refresh import (
...
class ProfileSetPenaltyShadowRefreshTests(unittest.TestCase):
    def _args(self) -> argparse.Namespace:
        return argparse.Namespace(
            config="config.toml",
            name_prefix="penalty_shadow_test",
    ...
    def test_shadow_argv_contains_penalty_settings(self) -> None:
        argv = _shadow_argv(
            python_exe="python",
            args=self._args(),
            compare_csv="../PancakeBot_var_exp/example_compare.csv",
        )
        self.assertIn("inspection.run_profile_set_penalty_shadow_recommender", argv)
        self.assertIn("--flow-penalty-per-500", argv)
        self.assertIn("--stageg2-penalty-per-500", argv)
