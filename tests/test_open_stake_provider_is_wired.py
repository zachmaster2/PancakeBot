...
on 2026-08-28 (tripped on the raw reading when the settled
equivalent was under the bar, on a live-money account).
...
def _open_ledger(tmp_path: Path) -> str:
    ...
    p.write_text(
        json.dumps({"epoch": 410951, "status": "SUBMITTED",
                    "amount_bnb": STAKE, "side": "Bull"}) + "\n"
        + json.dumps({"epoch": 410951, "status": "CONFIRMED"}) + "\n",
        encoding="utf-8")
    return str(p)
...
def test_record_settlement_writes_the_open_stake_column(monkeypatch,
    ...
    pipe.set_bankroll_tracker(PersistedBankrollTracker(
        path=hist, initial_bankroll=2.5,
        drawdown_peak_window_days=7))
    pipe.record_settlement(bankroll=2.1, start_at=1_790_000_000)
...
def test_the_incident_numbers_do_not_trip_once_the_provider_works(monkeypatch,
                                                                  tmp_path):
    """End-to-end on synthetic figures shaped like the 2026-08-28 incident:
    raw 16% (trips), settled-equivalent 14% (must not)."""
    pipe = _live_pipeline(monkeypatch, _open_ledger(tmp_path))
    peak, raw = 2.5, 2.1
    settled = raw + pipe._open_stake_bnb()
    assert (peak - raw) / peak == pytest.approx(0.16, abs=1e-5)
    assert (peak - settled) / peak == pytest.approx(0.14, abs=1e-5)
