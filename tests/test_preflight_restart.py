...
SKIP_LINE = ("2026-01-01 12:05:25.49  INFO   SKIP      Skipped epoch 409795: "
             "pool below minimum (1.09 BNB < 1.25 BNB threshold)")
BET_LINE = ("2026-01-01 12:00:20.50  INFO   BET       Bet 0.0500 BNB on Bear "
            "for epoch 409794 (tx 0x123456...)")
...
def _gate(monkeypatch, tmp_path, *, records, decision, slack,
    ...
    monkeypatch.setattr(pf, "lock_slack", lambda *a, **k: (409795, slack))
...
def test_confirmed_bet_counts_as_open(tmp_path):
    """THE CASE THAT GOT THROUGH. The live bet was CONFIRMED — not
    SUBMITTED — when the 13:44:37 restart went out. A hand-written test of
    'SUBMITTED minus terminal' misses this exactly."""
    path = _ledger(tmp_path, [
        {"epoch": 409794, "status": "SUBMITTED", "amount_bnb": 0.05},
        {"epoch": 409794, "status": "CONFIRMED", "gas_paid_bnb": 0.000119},
    ])
    assert pf.open_positions(path) == [(409794, "CONFIRMED")]


def test_late_is_terminal_and_not_an_open_position(tmp_path):
    """THE OTHER MISREAD. A live bet went LATE: the TX reverted and no
    position was ever taken, so it must NOT block a restart. An ad-hoc
    check flagged it and nearly stalled a correct deploy."""
    path = _ledger(tmp_path, [
        {"epoch": 409790, "status": "SUBMITTED", "amount_bnb": 0.05},
        {"epoch": 409790, "status": "LATE", "gas_paid_bnb": 3.6e-05},
    ])
...
def test_a_settled_history_leaves_nothing_open(tmp_path):
    path = _ledger(tmp_path, [
        {"epoch": 409788, "status": "SUBMITTED", "amount_bnb": 0.05},
        {"epoch": 409788, "status": "CONFIRMED"},
        {"epoch": 409788, "status": "SETTLED_LOST", "delta_bnb": -0.05},
        {"epoch": 409790, "status": "SUBMITTED", "amount_bnb": 0.05},
        {"epoch": 409790, "status": "LATE"},
    ])
    assert pf.open_positions(path) == []
...
def test_a_reworded_bet_line_is_caught_by_the_ledger_cross_check(
    ...
    path = _ledger(tmp_path, [
        {"epoch": 409800, "status": "SUBMITTED"},
        {"epoch": 409800, "status": "SETTLED_LOST"},
    ])
    monkeypatch.setattr(pf, "last_decision",
                        lambda *a, **k: ("SKIP", "older skip", 409795))
    monkeypatch.setattr(pf, "lock_slack", lambda *a, **k: (409801, 240.0))
    blockers = pf.check(unit="u", ledger_path=path, min_slack_s=90.0)
...
def test_slack_is_measured_against_the_open_round_lock():
    epoch, slack = pf.lock_slack(now=1000.0, next_lock=lambda: (409795, 1240))
    assert epoch == 409795
...
def test_gate_blocks_the_1344_restart(monkeypatch, tmp_path):
    ...
    blockers = _gate(
        monkeypatch, tmp_path,
        records=[{"epoch": 409794, "status": "SUBMITTED"},
                 {"epoch": 409794, "status": "CONFIRMED"}],
...
def test_main_exit_codes(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(pf, "last_decision",
                        lambda *a, **k: ("SKIP", "", 999999))
    monkeypatch.setattr(pf, "lock_slack", lambda *a, **k: (409795, 240.0))
