...
def test_shadow_settles_against_built_round_exact_math():
    ...
    from pancakebot.runtime.engine import build_shadow_settlement_round
    sl = ShadowLedger(path=None)
    sl.start(bankroll=1.9, start_at=1_790_000_000)
    sl.record_fire(epoch=700, side="Bear", size_bnb=0.095)
    ...
    # pools 4 bull / 6 bear + our 0.095 bear;
    # credit = 0.095 * (10.095*0.97/6.095) - claim_gas
    expected_credit = 0.095 * (10.095 * (1 - _FEE) / 6.095) - MAX_GAS_COST_CLAIM_BNB
    assert pnl == pytest.approx(expected_credit - 0.095 - MAX_GAS_COST_BET_BNB, abs=1e-9)
    assert sl.n_settled == 1 and sl.n_wins == 1
...
def test_build_settlement_round_one_sided_pool_no_zero_bets():
    ...
    expected_credit = 0.095 * ((4.0 + 0.095) * (1 - _FEE) / 0.095) - MAX_GAS_COST_CLAIM_BNB
    assert pnl == pytest.approx(
        expected_credit - 0.095 - MAX_GAS_COST_BET_BNB, abs=1e-9)
