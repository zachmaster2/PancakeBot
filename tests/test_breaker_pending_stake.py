...
That fired the live breaker on 2026-08-24 and suspended the
...
# --- the 2026-08-24 incident, on synthetic figures of the same shape -----
INCIDENT_PEAK = 2.2
INCIDENT_WALLET = 1.8645           # raw balance the breaker compared
INCIDENT_STAKE = 0.05              # the open position: CONFIRMED, later WON
THRESHOLD = 0.15
...
def test_the_incident_does_not_trip_once_the_open_stake_is_valued_at_cost():
    """THE regression. Peak 2.2, wallet 1.8645, one CONFIRMED
    0.05 position -> settled-equivalent 1.9145 -> 12.98%, no trip."""
    settled = INCIDENT_WALLET + INCIDENT_STAKE
    dd = _dd(INCIDENT_PEAK, settled)
    assert dd == pytest.approx(0.129773, abs=1e-5)
...
def test_a_late_bet_is_not_added_back(tmp_path):
    """The concrete case: a bet went LATE on 2026-08-24 (TX reverted, no
    position taken). Adding its stake back would overstate the bankroll."""
    path = _ledger(tmp_path, [
        {"epoch": 409821, "status": "SUBMITTED", "amount_bnb": 0.05},
        {"epoch": 409821, "status": "LATE", "gas_paid_bnb": 3.6e-05},
    ])
    assert open_stake_bnb(path) == 0.0
...
def test_the_correction_is_bounded_by_the_stake_in_flight():
    ...
    exactly the open stake, so at a 0.05 cap it is worth ~2.3pp of drawdown
    on a ~2.2 bankroll, not an open-ended allowance."""
...
def _decide(pipe):
    ...
    return pipe.decide_open_round(
        round_t=Round(epoch=409852, start_at=1790000880 - 300,
                      lock_at=1790000880, lock_price=None, close_price=None,
...
def test_pipeline_still_fires_when_the_loss_is_realised(tmp_path):
    ...
    assert d.skip_context["drawdown_pct"] == pytest.approx(15.25, abs=1e-3)
...
def test_the_suspension_baseline_is_seeded_on_the_settled_basis(tmp_path):
    ...
    recorded one open stake (0.05) too low.
...
def test_the_incident_declines_a_new_bet_without_suspending(tmp_path):
    """THE incident, exercising both halves at once. At the trip the raw
    wallet gave 15.25% >= 15%, so a new position was not survivable; the
    settled-equivalent gave 12.98%, so the account was not in trouble.
    ...
    assert d.skip_context["worst_case_pct"] == pytest.approx(15.25, abs=1e-3)
...
def test_the_lockout_releases_when_the_position_reaches_a_terminal_status(
    ...
    # the same wallet is now a genuine 15.25% drawdown, so it SUSPENDS.
...
def test_the_recorded_stake_makes_the_settled_basis_recomputable():
    ...
    Residual it closes: C*s/(P*(P+s)) ~ 1.9pp with one 0.05 stake in
    flight, ~3.8pp with two."""
    P, C, s = INCIDENT_PEAK, INCIDENT_WALLET + INCIDENT_STAKE, 0.05
    residual = C * s / (P * (P + s))
    assert residual == pytest.approx(0.0193, abs=5e-4)
    residual2 = C * (2 * s) / (P * (P + 2 * s))
    assert residual2 == pytest.approx(0.0378, abs=5e-4)
